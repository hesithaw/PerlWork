...
my $SCRIPT_NAME = "MGMT_UNIXUserIDFileGenerator";
my $LOG_FILE_NAME = $SYSTEM_NAME."_".$SCRIPT_NAME.".log";
my $USER_FILE_NAME = "../MQ_SERVER_SCRIPTS/MQ_SERVER_DATA_FILES/UNIXUserFile.txt";
my $LOG_FILE_FD;
my $USER_FILE_FD;
...
sub GetUsersfromDB{
	...
	my $membersNames = qx {sqlplus -s $ATS_DB_STRING<< EOF
                SELECT MCA_ID FROM ATSD_OPN_NORMAL WHERE MCA_ID IS NOT NULL;
                exit;
                EOF};
